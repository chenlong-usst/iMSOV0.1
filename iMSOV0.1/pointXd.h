...
public:
	...
	//绕点cp逆时针旋转ang度
	point2d Rotate(double ang, const point2d& cp);
...
//绕点cp逆时针旋转ang度
inline
point2d point2d::Rotate(double ang0, const point2d& cp)
{
	point2d v;
	double d = sqrt((x - cp.x)*(x - cp.x) + (y - cp.y)*(y - cp.y));
	double ang1 = acos((x - cp.x) / d);
...
inline 
bool operator < (const point3d& v1, const point3d& v2) //2023.5.18缺少<的重载
{
	double erf = 0.001;					//点的误差判断阈值
	if (v1.x - v2.x <0 && abs(v1.x - v2.x)>erf)
		return true;
	else if (abs(v1.x - v2.x) <= erf && v1.y - v2.y < 0 && abs(v1.y - v2.y)>erf)
		return true;
	else if (abs(v1.x - v2.x) <= erf && abs(v1.y - v2.y) <= erf && v1.z - v2.z <0 && abs(v1.z - v2.z)>erf)
		return true;
	return false;
}
